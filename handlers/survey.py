...
from db_module import record_survey_response
...
async def handle_survey_response(callback_query: types.CallbackQuery):
    ...
    # Здесь можно обработать ответ пользователя, например, записать его в базу данных
    record_survey_response(user_id, selected_option)

    await callback_query.answer(text=f"Вы выбрали: {selected_option}")
